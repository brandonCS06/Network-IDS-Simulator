...
"""
Ubuntu auth log parser for converting sshd authentication activity into the
canonical Events.json schema consumed by the Java IDS engine.

The parser currently emits brute-force-ready SSH login events. It supports
both traditional syslog timestamps, such as:
...
def main() -> int:
    ...
    parser.add_argument("--input", required=True, help="Path to Ubuntu auth log file.")
    parser.add_argument(
        "--output",
    ...
    try:
        events, skipped = parse_file(input_path, args.year)
...
def parse_line(line: str, line_number: int = 0, syslog_year: int | None = None) -> Event | None:
    ...
    if not parsed.process.startswith("sshd"):
        LOGGER.debug("Skipping non-sshd line %s from %s", line_number, parsed.process)
        return None
